...
- **E2E**: `@playwright/test` via `apps/web/playwright.config.ts` (Chromium, port 4173, `strictPort: true`). Requires `bun run build` first. Reuses preview server (`reuseExistingServer: true`).
...
- `docs/product/product-core.md` — Core library product scope, public API, and constraints
