...
- **Web preview binary**: `PreviewBody` routes into image / text / hidden. Image via blob URL; text via sync `TextDecoder` + `hljs.highlight` (no size cap, no deferred). All Unity-generated YAML extensions return `'unsupported'` from `getPreviewKindForPath` so preview frame collapses. Only `yaml`/`yml`/`meta` and code/doc extensions get `'text'`. Source of truth: `docs/reference/extension-map.md`.
