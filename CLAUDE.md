...
- **Web preview binary**: `PreviewBody` routes into image / text / hidden. Image via blob URL; text via sync `TextDecoder` + `hljs.highlight` (no size cap, no deferred). Current source behavior keeps Unity-generated YAML extensions as `'unsupported'` from `getPreviewKindForPath`, so the preview frame collapses until `docs/plans/web/preview-feature-tailoring.md` lands. Only `yaml`/`yml`/`meta` and code/doc extensions get `'text'`. Source of truth: `docs/reference/extension-map.md`.
- **`isUnityYamlBinary` (core)**: content-based detector for Unity YAML payloads, exported from `unitypackage-core`. Combines `%YAML` magic-byte check with a head+tail line-length scan (32 KB windows, 2048-byte max line). Still exported but **no longer called** by `getPreviewKindForPath` -- preview is purely extension-based. Useful for downstream consumers (CLI, tools) that need to classify content.
...
- **`TextPreview` is fully synchronous**: `TextDecoder.decode` + `hljs.highlight` run on the main thread with no chunking or Web Worker offload. A multi-MB string freezes the tab for 5--30 seconds. Preview kind is now extension-only (no size cap), so large `.yaml`/`.yml`/code/meta files are rendered directly -- only Unity YAML extensions are hidden. Avoid attaching giant text files to a test package.
