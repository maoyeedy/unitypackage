...
- `classify.ts` exposes content-aware helpers used by consumers to route assets: `getPreviewKindForPath(pathname, bytes?)`, `getMimeTypeForPath(pathname)`, `getSyntaxLanguageForPath(pathname)`, and `isUnityYamlBinary(bytes)`. The last is a content sniff that combines a `%YAML` magic-byte check with a head+tail line-length scan (32 KB windows, 2048-byte max line) to distinguish pure-text YAML from Force-Text serialized YAML that embeds binary blobs (textures, font atlases, lightmap data, shader variants). Filename patterns from `gitattributes.md` are not used.
